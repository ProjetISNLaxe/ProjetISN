...
def shooter(fenetre, glitch):
    classes_shooter.pygame.key.set_repeat(200, 15)  # refrappe
    clock = classes_shooter.pygame.time.Clock()  # horloge interne
    ...
    while 1:  # les deplacements
        while perso.vie != -1:
            for event in classes_shooter.pygame.event.get():
                if event.type == constantes.QUIT:  # quitter le jeux en cliquant sur la croix
                    classes_shooter.pygame.quit()
                    sys.exit()
                if event.type == constantes.KEYDOWN:
                    if event.key == constantes.K_ESCAPE:  # Echap pour quitter
                        classes_shooter.pygame.quit()
                        sys.exit()
                    # les deplacements

                    if event.key == constantes.PAUSE:  # Si on met en Pause
                        pause = True
                        while pause:
                            classes_shooter.pygame.time.delay(1000)  # Delai pour ne pas qu'il  l'appuie de la touche
                            for event in classes_shooter.pygame.event.get():
                                if event.type == constantes.QUIT:  # Pour pouvoir tout de  quitter
                                    classes_shooter.pygame.quit()
                                    sys.exit()
                                if event.type == constantes.KEYDOWN:
                                    if event.key == constantes.K_ESCAPE:
                                        perso.vie = 0
                                    if event.key == constantes.K_p:  # On  a la pause si on appuie sur p
                                        pause = False
            ...
            if ennemi_1.incollision and score < constantes.SCORE_MINIBOSS:  # Collision ennemi perso
            ...
            if ennemi_2.incollision and constantes.SCORE_MINIBOSS > score >= constantes.SCORE_ENNEMI_2:  # Collision ennemi perso
                perso.inexplosion = True
                perso.explosion_rect = Rect(perso.rect.x - 25, perso.rect.y - 25, 192, 192)
                ennemi_2.inexplosion = True  # On anime l'explosion
                ennemi_2.explosion_rect = Rect(ennemi_2.rect.x - 25, ennemi_2.rect.y - 25, 192, 192)
            ...
            for q in range(ennemi_2.nbrTir):
                if perso.mask.overlap(tirennemi_mask, (ennemi_2.tirx[q] - perso.rect.x, ennemi_2.tiry[
                                                                                            q] - perso.rect.y)) and score >= constantes.SCORE_ENNEMI_2:  # colision entre projectille et perso
            ...
            if classes_shooter.pygame.sprite.collide_mask(ennemi_1,
                                                          ennemi_2) and constantes.SCORE_ENNEMI_2 <= score < constantes.SCORE_MINIBOSS and ennemi_1.rect.y > 0:  # colision entre ennemi 1 et 2 (que si l'ennemi 2 existe)
                ennemi_1.inexplosion = True
                ennemi_1.explosion_rect = Rect(ennemi_1.rect.x - 25, ennemi_1.rect.y - 25, 192, 192)
                ennemi_1.respawn()
                ennemi_2.respawn()
            ...
            for n in range(perso.nbrTir):
                if miniboss.maskcanon.overlap(tir_mask, (perso.tirx[n] - miniboss.rectcanon[0],
                                                         perso.tiry[n] - miniboss.rectcanon[
                                                             1])) and score >= constantes.SCORE_MINIBOSS and miniboss.canonalive == True:
            ...
            for w in range(perso.nbrTir):
                if miniboss.maskcanoncasse.overlap(tir_mask, (perso.tirx[w] - miniboss.rectcanon[0],
                                                              perso.tiry[w] - miniboss.rectcanon[
                                                                  1])) and score >= constantes.SCORE_MINIBOSS and miniboss.canonalive == False:
                    perso.tirx[w] = 10000
            for g in range(perso.nbrTir):
                if miniboss.mask.overlap(tir_mask, (perso.tirx[g] - miniboss.rect.x, perso.tiry[
                                                                                         g] - miniboss.rect.y)) and score >= constantes.SCORE_MINIBOSS and miniboss.alive == True:
            ...
            if score == constantes.SCORE_MINIBOSS - 1:
                if deathennnemi:
                    ennemi_1.inexplosion = True
                    ennemi_1.explosion_rect = Rect(ennemi_1.rect.x - 25, ennemi_1.rect.y - 25, 192, 192)
                    ennemi_2.inexplosion = True  # On anime l'explosion
            ...
            if score >= constantes.SCORE_MINIBOSS - 1 and miniboss.alive == True:
            ...
            if ennemi_2.nbrTir > 0 and ennemi_2.tir_ok == True and constantes.SCORE_MINIBOSS - 1 > score >= constantes.SCORE_ENNEMI_2:  # On affiche tous les tirs ennemis si ils existent
            ...
            if 0 <= score < constantes.SCORE_MINIBOSS - 1:
                ennemi_1.alive = True
                fenetre.blit(ennemi_1.image, ennemi_1.rect)

            if constantes.SCORE_ENNEMI_2 <= score < constantes.SCORE_MINIBOSS - 1:  # On affiche l'ennemi 2 que si on a 10 en score
                ennemi_2.alive = True
            ...
            if score > constantes.SCORE_MINIBOSS and miniboss.mask.overlap(perso.mask, (perso.rect.x - miniboss.rect.x,
                ...
                perso.rect.y += constantes.VITESSE_PERSO
                if not perso.immortel:
                    perso.inexplosion = True
                    perso.explosion_rect = Rect(perso.rect.x - 25, perso.rect.y - 25, 192, 192)
                    perso.vie -= 1  # On perd une vie si on est pas immortel
                    perso.immortel = True
            if score > constantes.SCORE_MINIBOSS and miniboss.maskcanon.overlap(perso.mask,
                ...
                if not perso.immortel:
                    ...
                    perso.vie -= 1  # On perd une vie si on est pas immortel
                    perso.immortel = True
            elif score > constantes.SCORE_MINIBOSS and miniboss.maskcanon.overlap(perso.mask,
                                                                                  (perso.rect.x - miniboss.rectcanon.x,
                ...
                perso.rect.x += constantes.VITESSE_PERSO
                if not perso.immortel:
                    perso.inexplosion = True
                    perso.explosion_rect = Rect(perso.rect.x - 25, perso.rect.y - 25, 192, 192)
                    perso.vie -= 1  # On perd une vie si on est pas immortel
            ...
            classes_shooter.pygame.display.flip()  # On raffraichis l'ecran
            clock.tick(60)  # 60 FPS

        classes_shooter.pygame.display.flip()
        return
...
def endless(fenetre, glitch):
    ...
    groupesprite = classes_shooter.pygame.sprite.Group()
    fenetre_taille = fenetre.get_size()
    fond = classes_shooter.imfond(fenetre_taille)  # charge l'image de fond
        ...
        while perso.vie != -1:
            ...
            if perso.mask.overlap(miniboss.boulemask2, (miniboss.boulerect2.x - perso.rect.x,
                ...
                if not perso.immortel:
                    ...
                    perso.immortel = True
            if perso.mask.overlap(miniboss.lasermask, (miniboss.laserrect.x - perso.rect.x,
                                                       miniboss.laserrect.y - perso.rect.y)) and miniboss.laseer == True and miniboss.canonalive == False and score > constantes.SCORE_MINIBOSS:  # colision boule miniboss perso
                if not perso.immortel:
                    perso.inexplosion = True
                    perso.explosion_rect = Rect(perso.rect.x - 25, perso.rect.y - 25, 192, 192)
                    perso.vie -= 1  # On perd une vie si on est pas immortel
            ...
            if miniboss.laseer == True and score > constantes.SCORE_MINIBOSS:
                fenetre.blit(miniboss.laserimage, (miniboss.boulerect2[0] - 4, miniboss.boulerect2[1] + 10))
            if miniboss.laser == True and miniboss.attaque1_moveinvert == False and miniboss.attaque1_movevert == False and score > constantes.SCORE_MINIBOSS:
                fenetre.blit(miniboss.bouleimage2, miniboss.boulerect2)
            ...
            if ennemi_2.nbrTir > 0 and ennemi_2.tir_ok == True and constantes.SCORE_MINIBOSS > score >= constantes.SCORE_ENNEMI_2:  # On affiche tous les tirs ennemis si ils existent
                for i in range(ennemi_2.nbrTir):
                    ennemi_2.tiry[i] += 6
                    fenetre.blit(tirennemi, (ennemi_2.tirx[i], ennemi_2.tiry[i]))
            ...
            if 0 <= score < constantes.SCORE_MINIBOSS:
                ennemi_1.alive = True
                fenetre.blit(ennemi_1.image, ennemi_1.rect)

            if constantes.SCORE_ENNEMI_2 <= score < constantes.SCORE_MINIBOSS:  # On affiche l'ennemi 2 que si on a 10 en score
                ennemi_2.alive = True
                fenetre.blit(ennemi_2.image, ennemi_2.rect)
            elif score >= constantes.SCORE_MINIBOSS:
                ennemi_2.alive = False
                ennemi_1.alive = False
            if score >= constantes.SCORE_MINIBOSS:
                for z in range(miniboss.nbrTir):
                    miniboss.tiry[z] += 10
                    fenetre.blit(tirennemi, (miniboss.tirx[z], miniboss.tiry[z]))
            ...
            if miniboss.energiebowl == True and miniboss.canonalive == True and score > constantes.SCORE_MINIBOSS:
                miniboss.boulemovex = (perso.rect.x + perso.size[0] / 2 - 250) / 75
                miniboss.boulemovey = (110 + perso.rect.y) / 75
                fenetre.blit(miniboss.bouleimage, miniboss.boulerect)
            if miniboss.energiebowl1 == True and miniboss.canonalive == True and score > constantes.SCORE_MINIBOSS:
                miniboss.boulemove1x = (perso.rect.x + perso.size[0] / 2 - 515) / 75
                miniboss.boulemove1y = (110 + perso.rect.y) / 75
                fenetre.blit(miniboss.bouleimage1, miniboss.boulerect1)
