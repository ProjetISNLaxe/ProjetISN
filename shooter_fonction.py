...
def shooter(fenetre, glitch):
    ...
    while 1:  # les deplacements
        while perso.vie != -1:
            ...
            if ennemi_2.nbrTir > 0 and ennemi_2.tir_ok == True and score < SCORE_MINIBOSS-1 and score>=SCORE_ENNEMI_2:  # On affiche tous les tirs ennemis si ils existent
            ...
            if score >= 0 and score < SCORE_MINIBOSS-1:
                ennemi_1.alive = True
                fenetre.blit(ennemi_1.image, ennemi_1.rect)

            if score >= SCORE_ENNEMI_2 and score < SCORE_MINIBOSS-1:  # On affiche l'ennemi 2 que si on a 10 en score
                ennemi_2.alive = True
            ...
            if score > SCORE_MINIBOSS and miniboss.mask.overlap(perso.mask, (perso.rect.x - miniboss.rect.x,
                ...
                if perso.immortel == False:
                    ...
                    perso.vie -= 1  # On perd une vie si on est pas immortel
                    perso.immortel = True
            if score > SCORE_MINIBOSS and miniboss.maskcanon.overlap(perso.mask, (perso.rect.x - miniboss.rectcanon.x,
                ...
                if perso.immortel == False:
                    ...
                    perso.vie -= 1  # On perd une vie si on est pas immortel
                    perso.immortel = True
            elif score > SCORE_MINIBOSS and miniboss.maskcanon.overlap(perso.mask, (perso.rect.x - miniboss.rectcanon.x,
...
def endless(fenetre, glitch):
    ...
    score = SCORE_BASE  # Nombre d'ennemis mort
    highscore = open("endless\highscore.txt", "r")
    ...
    while 1:  # les deplacements
        while perso.vie != -1:
                ...
                if miniboss.mask.overlap(tir_mask, (perso.tirx[g] - miniboss.rect.x, perso.tiry[g] - miniboss.rect.y)) and score >= SCORE_MINIBOSS and miniboss.alive == True:
                    if miniboss.canonalive == False:
                        if miniboss.ptVie > 1:
                            ...
                            miniboss.rectcanon=Rect(fenetre_taille[0]/2-199,miniboss.rect.y+49,miniboss.sizecanon[0],miniboss.sizecanon[1])
                            miniboss.rect=Rect(fenetre_taille[0]/2-miniboss.size[0]/2, -miniboss.size[1]-50, miniboss.size[0], miniboss.size[1])
            ...
            if perso.vie == 0:
                ...
                if score+compteur > int(highscoretxt) and test==True:
                    fenetre.blit(myfontdeath.render("New Best !!!", False, (255, 255, 255)),
                                 (300, 350))
                    chaine="New Best !!!"
                    highscore=open("endless\highscore.txt", "w")
            ...
            if ennemi_2.nbrTir > 0 and ennemi_2.tir_ok == True and score < SCORE_MINIBOSS and score>=SCORE_ENNEMI_2:  # On affiche tous les tirs ennemis si ils existent
                for i in range(ennemi_2.nbrTir):
                    ennemi_2.tiry[i] += 6
                    fenetre.blit(tirennemi, (ennemi_2.tirx[i], ennemi_2.tiry[i]))
