...
class personnage(pygame.sprite.Sprite):
    ...
    def explosionanim(self):  # animation de l'explosion
        if self.inexplosion==True:
            self.test=True
            self.inexplosion=False
        if self.test==True:
            self.explosionact=True
        if self.explosionact==True:
            self.p+=1
            if 1<self.p<5:
                self.explosion=self.explosionim[0]
            elif 5<self.p<10:
...
class ennemi1(ennemi):
    def __init__(self, fenetre_taille):
        ennemi.__init__(self, "ennemi\ennemi_1.png")
        self.alive=False
    ...
    def move(self, score):
        ...
        elif self.alive==True:  #Si l'ennemie est en dessous de l'ecrans on le respawn en haut
...
class ennemi2(ennemi):
    def __init__(self, fenetre_taille):
        ennemi.__init__(self, "ennemi\ennemi_2.png")
        self.alive=False
    ...
    def move(self, score):
        if self.rect.y<=100 and self.alive==True: #Si l'ennemi 2 n'a pas atteint y=100 on le fait avancer
                self.rect.y+=VITESSE_ENNEMI_2_Y
           ...
           if self.rect.x>800:  #Si l'ennemi est au bord on le respawn de l'autre
               ...
               self.rect.x+=VITESSE_ENNEMI_2_X
    def collision(self, perso):
        if pygame.sprite.collide_mask(self, perso):
            self.incollision=True
...
class imfond(pygame.sprite.Sprite):
    def __init__(self, fenetre_taille):
        self.image1=pygame.image.load("HUD\space.png").convert()
        self.image2=pygame.image.load("HUD\space.png").convert()
...
class clminiboss(pygame.sprite.Sprite):
    def __init__(self, fenetre_taille):
        super().__init__()
        self.image = pygame.image.load("ennemi\miniboss\mini_boss.png").convert_alpha()
        self.rect = self.image.get_rect()
        self.size=self.image.get_size()
        ...
        self.tiry=[]
        self.nbrTir=0
        self.imgcanon=pygame.image.load("ennemi\miniboss\miniboss_ailes.png").convert_alpha()
        self.imgcanoncasse=pygame.image.load("ennemi\miniboss\Boss wings broken.png").convert_alpha()
        ...
        self.rectcanon=Rect(fenetre_taille[0]/2-199,49,self.sizecanon[0],self.sizecanon[1])
        ...
        for k in range (3):
            bouleimage="ennemi\\miniboss\\sprite_energyball"+str(k)+".png"
        ...
        self.boulemaskli2=[]
        for m in range (3):
            bouleimage="ennemi\\miniboss\\sprite_energyball"+str(m)+".png"
       ...
       #Eclairimage
        ...
        self.eclairmaskli=[]
        for i in range(5):
            eclairnom="ennemi\\miniboss\\eclair\\eclair_"+str(i+1)+".png"
    ...
    def attaque1(self, score):
        if score==SCORE_MINIBOSS+5 and self.test==True:
            ...
            self.test=False
            self.unlimitedpoweer=False
        if self.attaque1_movevert==True:
            self.rect.y+=10
            self.rectcanon.y+=10
            if self.rect.y>=600:
                self.attaque1_movevert=False
                self.attaque1_moveinvert=True
        if self.attaque1_moveinvert==True:
    ...
    def attaque3(self, score):
        if self.test2==True and score>=80:
    ...
    def attaque4(self, score, increment):
        if score>=SCORE_MINIBOSS+5 and self.attaque1_moveinvert==False and self.attaque1_movevert==False and self.test3==True:
            self.energiebowl=True
            self.test3=False
        if self.energiebowl==True :
            self.bouleload=True
        if self.bouleload==True:
    ...
    def attaque5(self, score, increment):
        if score>=SCORE_MINIBOSS+5  and self.attaque1_moveinvert==False and self.attaque1_movevert==False and self.test4==True:
            ...
            self.test4=False
        if self.energiebowl1==True:
            self.bouleload1=True
        if self.bouleload1==True:
    ...
    def attaque6(self, score, increment,perso):
        if self.canonalive==False and self.test5==True:
            self.laser=True
            self.laserload=True
            self.test5=False
        if self.laserload==True:
            print("ok")
